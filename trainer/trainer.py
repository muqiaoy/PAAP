...
from .paap_loss import PAAPLoss
from .paap_loss import AcousticEstimator
...
class Trainer(object):
    def __init__(self, data, model, optimizer, args):
        ...
        self.ac_loss = PAAPLoss(args = args).to(self.device)
        ...
        if load_from:
            ...
            if continue_pretrained is not None:
                ...
                if self.args.model == "Demucs":
                    state_dict = torch.load(continue_pretrained)
                    self.model.load_state_dict(state_dict)
                elif self.args.model == 'FullSubNet':
                    state_dict = torch.load(continue_pretrained, map_location=torch.device(torch.cuda.current_device()))["model"]
                    self.model.load_state_dict(state_dict)
                else:
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in tqdm(enumerate(logprog)):
            ...
            noisy = data[0]
            clean = data[1]
            if not cross_valid and self.args.model == 'Demucs':
            ...
            with torch.autograd.set_detect_anomaly(True):
                if not self.args.acoustic_loss_only: 
                    if self.args.model == 'Demucs':
                        ...
                        estimate, enh_loss = self.dmodel(noisy, clean)
                    else:
                        raise NotImplementedError
