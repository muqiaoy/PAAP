...
from datasets.audio import Audioset, find_audio_files
from enh.demucs.demucs import DemucsStreamer
from . import distrib
...
def save_wavs(estimates, noisy_sigs, filenames, out_dir, sr=16_000, save_noisy=False):
    # Write result
...
def enhance(args, model=None, local_out_dir=None):
    ...
    if not hasattr(model, 'chin'):
        num_channels = 1  # TODO
    else:
        num_channels = model.chin
    dset = get_dataset(args, model.sample_rate, num_channels)
